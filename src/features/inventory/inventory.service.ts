 ...
 *
 * Implementación SIMULADA sobre `inventoryStore` y `adjustmentsStore`. Aplica las mismas
 * reglas que el formulario (`adjustments.ts`) porque el backend real tampoco debe confiar
 * en la validación del cliente. Para conectar el backend, reemplaza el cuerpo de cada método
 * por la llamada HTTP indicada (ver `API_ENDPOINTS.inventory`) y elimina `inventory.mock.ts`;
