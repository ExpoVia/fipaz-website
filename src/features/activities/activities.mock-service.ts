...
/**
 * Servicio SIMULADO de actividades y asistencia (`NEXT_PUBLIC_API_MODE=mock`, el valor por defecto).
 *
 * Trabaja sobre `activitiesStore` y `attendanceStore` (memoria + sessionStorage). Los contadores
 * `registered` y `attendees` se recalculan desde los registros de asistencia, como lo haría el
 * backend. La implementación contra el backend está en `activities.http-service.ts`;
 * `activities.service.ts` elige cuál se usa.
 */
...
export const activitiesMockService: ActivitiesService = {
